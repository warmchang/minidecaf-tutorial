...
2026 年秋季学期基本沿用了往年《编译原理》课程的语法规范。为了贴合课程教学内容，提升训练效果，课程组设计了比较完善的编译器框架，包括词法分析、语法分析、语义分析、中间代码生成、数据流分析、寄存器分配、目标平台汇编代码生成等步骤。每个 step 同学们都会面对一个完整的编译器流程，但不必担心，实验开始的几个 step 涉及的编译器框架知识都比较初级，随着课程实验的深入，将会循序渐进地引入各个编译器功能模块，并通过文档对相关技术进行分析介绍，便于同学们实现相关编译功能模块。

本学期实验内容较大的一个改动是，我们不再提供完整评测数据集，而只提供几个最基础的测例供同学们参考。我们希望同学们能够主动思考可能的边界条件，而不是单纯面向测例编程。不过，你依然能够在 GitLab CI 页面看到本次实验获得的分数，从而知晓自己是否有遗漏某些边界情况。

本实验鼓励同学们使用智能编程工具，尤其鼓励同学们主动探索高效的交互模式，并在报告中有所体现。但你应当确保自己读懂、理解了智能体输出的代码，而不是盲目地复制粘贴。**期末考试中将出现对于实验内容的考察。**

## 实验起点和基本要求

本次实验一共设置 14 个步骤（其中 step 0 和 step 1 为实验框架熟悉，不需要修改框架代码）。后续的 step 2-13 我们将由易到难完成 MiniDecaf 语言的所有特性，由于编译器的边界情况很多，你**只需通过 CI 上评测用的正例与负例即可**。此外，parser step 为同学们提供了一个上手语法分析器的机会。
...
你需要通过 stage1 - stage6 与 parser-stage，以拿到总评中一定的分数（20%）。
...
大家在网络学堂提交 **git.tsinghua.edu.cn** 的帐号名后，助教会给每个人建立一个私有的仓库，URL 为 https://git.tsinghua.edu.cn/compiler26spring/stu26s/minidecaf-你的学号 ，将作业提交到那个仓库即可。
