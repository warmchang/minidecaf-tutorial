...
你需要自行设计一些测例来表现出你的寄存器分配算法的优化效果。只要你的代码能够通过之前章节的所有测例（即你在本 stage 没有引入 bug），并且能够在你的测例上表现出优化效果（如减少了寄存器 spill 的次数），即可得到本 stage 的全部分数。
我们还会在不公开的评测集上对所有同学的代码进行评测（不会体现在 CI 中），其中优化合理且效果显著优秀的同学将获得总评上的额外加分。
